...
def stopWords(nombre_archivo):
	archivo = open(nombre_archivo)
	archivo_csv = csv.reader(archivo)
	lista = []
	
	for palabra in archivo_csv:
...
def clean(s, lista):
	...
	review_limpio = ""
	s = " ".join(re.findall(r'\w+', s,flags = re.UNICODE | re.LOCALE)).lower()

	for palabra in s.split():
		if not(palabra in lista):
			if len(palabra) > 2:
				if (not (palabra[0].isdigit())):
					review_limpio += palabra + ' '
	return review_limpio
...
def get_data_tsv(loc_dataset,opts):
	...
	for e, line in enumerate(open(loc_dataset,"rb")):
		if e > 0:
			...
			if opts["clean"]:
				try:
					r[2] = clean(r[2], opts["StopWords"])
				except:
					r[1] = clean(r[1], opts["StopWords"])
...
if __name__ == "__main__":
	...
	opts["StopWords"] = stopWords("/home/guillermo/Documentos/StopWords.txt")

	#training and saving model into weights
	weights = train_tron("/home/guillermo/Documentos/labeledTrainData.tsv",opts)
	
	#testing and saving predictions into preds
	preds = test_tron("/home/guillermo/Documentos/testData.tsv",weights,opts)

	#writing kaggle submission
	with open("/home/guillermo/Documentos/popcorn.csv","wb") as outfile:
